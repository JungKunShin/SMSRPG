...
void UC_WrKickAN::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation)
{
	...
	if (Warrior->HasAuthority()) // 서버에서만 소환
	{
		...
		FVector SpawnLocation = Warrior->GetActorLocation() + ForwardVector * 150.0f; // 전사 위치에서 2미터 앞
		FRotator SpawnRotation = Warrior->GetActorRotation(); // 전사 각도 저장

		FActorSpawnParameters SpawnParam;
		SpawnParam.Owner = Warrior;
		SpawnedAttackParticle = MeshComp->GetWorld()->SpawnActor<AC_WrKickAP>(AttackParticleClass, SpawnLocation, SpawnRotation, SpawnParam); // 액터 소환
	}
}
