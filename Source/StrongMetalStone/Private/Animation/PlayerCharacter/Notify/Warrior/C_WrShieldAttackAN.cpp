...
#include "Animation/PlayerCharacter/Notify/Warrior/C_WrShieldAttackAN.h"
#include "Character/C_Warrior.h"
#include "PlayerController/C_WorldPlayerController.h"

void UC_WrShieldAttackAN::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation)
{
	...
	if (Warrior->HasAuthority()) // 서버에서만 소환
	{
		...
		FVector SpawnLocation = Warrior->GetActorLocation() + ForwardVector * 200.0f; // 전사 위치에서 2미터 앞
		FRotator SpawnRotation = Warrior->GetActorRotation(); // 전사 각도 저장

		FActorSpawnParameters SpawnParam;
		SpawnParam.Owner = Warrior;
		SpawnedAttackParticle = MeshComp->GetWorld()->SpawnActor<AC_WrShieldAttackAP>(AttackParticleClass, SpawnLocation, SpawnRotation, SpawnParam); // 액터 소환
	}
}
