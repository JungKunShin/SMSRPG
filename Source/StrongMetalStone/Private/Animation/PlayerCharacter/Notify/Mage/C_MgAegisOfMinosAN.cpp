...
void UC_MgAegisOfMinosAN::Notify(USkeletalMeshComponent* MeshComp, UAnimSequenceBase* Animation)
{
	...
	if (Mage->HasAuthority()) // 서버에서만 소환
	{
		...
		FVector SpawnLocation = Mage->GetActorLocation() + ForwardVector * 200.0f; // 마법사 위치에서 2미터 앞
		FRotator SpawnRotation = Mage->GetActorRotation(); // 마법사 각도 저장

		FActorSpawnParameters SpawnParam;
		SpawnParam.Owner = Mage;
		SpawnedAttackParticle = MeshComp->GetWorld()->SpawnActor<AC_MgAegisOfMinosAP>(AttackParticleClass, SpawnLocation, SpawnRotation, SpawnParam); // 액터 소환
	}
}
